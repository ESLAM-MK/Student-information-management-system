    ...
    axios
      ...
      .then((response) => {
        ...
        if (response.data.success) {
          ...
          let FilterSubject = response.data.subject.filter((e)=>e.branch === userData.branch)
          console.log(response.data.subject);
      ...
      <div className="mt-8 w-full flex justify-center items-center flex-col">
        <div className="flex justify-center items-center w-[40%]">
          ...
          <button
            ...
            className="bg-gray-700 text-white py-3 px-4 text-2xl rounded-sm"
          >
            <HiOutlineSearch />
          </button>
        </div>
        <div className="mt-8 w-full">
          ...
          {material && material.length === 0 && selected && (
            <p className="text-center text-yellow-400">No Material For {selected}!</p>
          )}
        </div>
      </div>
    </div>
